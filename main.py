...
def gameLoop():		
	...
	while not gameExit: #Stay in this loop unless the game is quit
		...
		tBot.updateBoard(copy.deepcopy(mainBoard.blockMat), mainBoard.piece, mainBoard.nextPieces, mainBoard.gameStatus, key, gameDisplay)
		...
		if mainBoard.gameStatus == "gameOver":
			...
			if mainBoard.score >= bestTestScore:
				with open("high-scores.txt", "a") as f:
					...
					f.write(f"avgHeight: {tBot.avgHeightWeight}\t\tbumpiness: {tBot.bumpinessWeight}\t\tholes: {tBot.holesWeight}\t\tlines: {tBot.lineWeight}\n")
